...
def keypoints_to_heat_map_rotate(keypoints, rois, heatmap_size, labels):
    ...
    keypoints_ = keypoints.view((-1, 8))
    keypoints_ = keypoints_.cuda().data.cpu().numpy()
    labels_ = labels.view((keypoints_.shape[0], 1))
    labels_ = labels_.cuda().data.cpu().numpy()
    ...
    for index, tran, keypoint, label in zip(range(0, rois_.shape[0]), trans, keypoints_, labels_):
        ...
        target = np.zeros((30, 56, 56))
        target_sum_corner = target_corner_[0]
        for m in range(target_corner_.shape[0]):
            target_sum_corner = np.maximum(target_sum_corner, target_corner_[m])
        target[label[0] - 1] = target_sum_corner

        target_sum_line = target_line_[0]
        for m in range(target_line_.shape[0]):
            target_sum_line = np.maximum(target_sum_line, target_line_[m]) 
        target[label[0] + 14] = target_sum_line
        ...
        keypoints_per_image[index] = target
        # keypoints_weight_per_image[index] = target_weight
    #print(keypoints_per_image.shape)
    keypoints_per_image = torch.as_tensor(keypoints_per_image, dtype=torch.float32, device=keypoints.device)
