...
class KeypointPostProcessor(nn.Module):
    ...
    def forward(self, x, boxes):
        ...
        scores = None
        if self.keypointer:
            mask_prob, scores = self.keypointer(x, boxes, labels)
...
def heatmaps_to_keypoints_affine(maps, rois, labels):
    """Extract predicted keypoint locations from heatmaps. Output has shape
    (#rois, 4, #keypoints) with the 4 rows corresponding to (x, y, logit, prob)
    for each keypoint.
    """
...
class Keypointer(object):
    ...
    def __call__(self, masks, boxes, labels):
        ...
        result, scores = heatmaps_to_keypoints_affine(
            masks.detach().cpu().numpy(), boxes[0].bbox.detach().cpu().numpy(), labels.detach().cpu().numpy()
        )
