...
def project_keypoints_to_heatmap(keypoints, proposals, discretization_size, label):
    proposals = proposals.convert("xyxy")
    return keypoints_to_heat_map_rotate(
        keypoints, proposals.bbox, discretization_size, label
        )
...
class KeypointRCNNLossComputation(object):
    ...
    def __call__(self, proposals, keypoint_logits):
        ...
        for proposals_per_image in proposals:
            label = proposals_per_image.get_field("labels")
            kp = proposals_per_image.get_field("keypoints")

            heatmaps_per_image = project_keypoints_to_heatmap(
                kp, proposals_per_image, self.discretization_size, label
            )
        ...
        keypoint_loss = F.binary_cross_entropy_with_logits(keypoint_logits, keypoint_targets) * 500
